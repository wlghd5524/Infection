    ...
    void Start()
    {
        ...
        for (int i = 0; i < ObjectPoolingManager.Instance.maxOfNurse; i++) // 최대 간호사 수만큼 루프 실행
        {
            if (i % 20 < 10) // 조건에 따라 특정 간호사만 활성화
            {
                ...
                {
                    case 0:
                        newNurseController.waypoints.Add(ward.transform.Find("PatientRoom").GetComponent<Waypoint>()); // 환자실 웨이포인트 추가
                    ...
                    case 3:
                        for (int j = 0; j < 5; j++)
                        {
                            newNurseController.waypoints.Add(ward.transform.Find("Doctor'sOffice (" + j + ")").GetComponent<Waypoint>()); // 의사실 웨이포인트 추가
                        }
                        break;
                    case 4:
                    case 5:
                    case 6:
                    case 7:
                        newNurseController.waypoints.Add(ward.transform.Find("Counter").GetComponent<Waypoint>()); // 카운터 웨이포인트 추가
                        break;
                    case 8:
                    case 9:
                        newNurseController.waypoints.Add(GameObject.Find("OutpatientWaypoints").transform.Find("Ward (" + i / 20 + ")").transform.Find("CounterWaypoint (0)").GetComponent<Waypoint>()); // 외래환자 카운터 웨이포인트 추가
                        newNurseController.waypoints.Add(GameObject.Find("OutpatientWaypoints").transform.Find("Ward (" + i / 20 + ")").transform.Find("SofaWaypoint (0)").GetComponent<Waypoint>()); // 외래환자 소파 웨이포인트 추가
                        break;
                }
            }
        }
